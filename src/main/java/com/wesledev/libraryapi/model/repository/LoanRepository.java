...
//   "from Loan l where l.book = :book and (l.returned is null or l.returned is false)")
    @Query("select case when (count(l.id) > 0) then true else false end from Loan l where l.book = :book and (l.returned is null or not l.returned)")
    ...
    @Query("select l from Loan l where l.loanDate <= :threeDaysAgo and (l.returned is null or not l.returned)")
